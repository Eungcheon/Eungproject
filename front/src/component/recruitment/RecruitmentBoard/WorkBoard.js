...
import "../RecruitmentBoard/WorkBoardPage.css"; // CSS 파일 유지
import "../RecruitmentBoard/WorkBoard.css";
...
function WorkBoard() {
  ...
  const currentItems = WorkBoard.slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(WorkBoard.length / itemsPerPage);
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={{ flex: 1, padding: "70px" }}>
        {/* 검색 */}
        <table className="workBoard-search">
          <tbody>
            <tr>
              <td>
                <span className="workBoard-board-name">추천 채용공고</span>
              </td>
              <td>
                <select
                  className="workBoard-custom-select"
                  value={choiceVal}
              ...
              <td>
                <input
                  type="text"
                  className="workBoard-form-control"
                  placeholder="검색어"
                  value={searchVal}
              ...
              <td>
                <button type="button" className="workBoard-btn workBoard-btn-outline-secondary">
        ...
        {/* 게시판 */}
        <table className="workBoard-table workBoard-table-hover">
          <thead>
            <tr>
              <th>즐겨찾기</th>
              <th>번호</th>
          ...
          <tbody>
            {currentItems.map((bbs, idx) => (
              <tr key={bbs.boardId}>
                <td onClick={() => toggleFavorite(bbs.boardId)} style={{ textAlign: "center", cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <img src={favorites[bbs.boardId] ? Goldstar : Graystar} alt="즐겨찾기" style={{ width: "20px", height: "20px" }} />
                  </div>
                </td>
        ...
        {/* 페이지네이션 */}
        <div className="workBoard-pagination">
          <button onClick={handlePrevPage} disabled={currentPage === 1}>
            «
          </button>
          {Array.from({ length: totalPages }, (_, index) => (
            <span key={index} className={`workBoard-page-number ${currentPage === index + 1 ? "active" : ""}`} onClick={() => setCurrentPage(index + 1)}>
              {index + 1}
            </span>
          ))}
        ...
        {/* 글쓰기 버튼 추가 */}
        <div className="workBoard-write-container">
          <Link className="workBoard-card-button" to="/workWrite">
