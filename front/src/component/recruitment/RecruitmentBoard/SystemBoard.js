...
import "../RecruitmentBoard/SystemBoardPage.css";
import "../RecruitmentBoard/SystemBoard.css";
...
function SystemBoard() {
  const [SystemBoard, setSystemBoard] = useState([
  ...
  const currentItems = SystemBoard.slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(SystemBoard.length / itemsPerPage);
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={{ flex: 1, padding: "70px" }}>
        {/* 검색 */}
        <table className="systemBoard-search">
          <tbody>
            <tr>
              <td>
                <span className="systemBoard-board-name">취업준비 지원제도</span>
              </td>
              <td>
                <select
                  className="systemBoard-custom-select"
                  value={choiceVal}
              ...
              <td>
                <input
                  type="text"
                  className="systemBoard-form-control"
                  placeholder="검색어"
                  value={searchVal}
              ...
              <td>
                <button
                  type="button"
                  className="systemBoard-btn btn-outline-secondary"
                >
                  <i className="systemBoard-fas fa-search"></i> 검색
                </button>
              </td>
            </tr>
          </tbody>
        ...
        {/* 게시판 */}
        <table className="systemBoard-table table-hover">
          <thead>
            <tr>
              <th>즐겨찾기</th>
              <th>번호</th>
        ...
        {/* 페이지네이션 */}
        <div className="systemBoard-pagination">
          ...
          {Array.from({ length: totalPages }, (_, index) => (
            <span
              key={index}
              className={`systemBoard-page-number ${currentPage === index + 1 ? "active" : ""}`}
        ...
        {/* 글쓰기 버튼 */}
        <div
          className="systemBoard-my-5 centered-container"
          style={{ display: "flex", justifyContent: "center" }}
        >
          <Link className="systemBoard-card-button" to="/SystemWrite">
            <i className="systemBoard-fas fa-pen"></i> &nbsp; 글 쓰기
          </Link>
        </div>
      </div>
    </div>
