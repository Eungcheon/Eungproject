...
import "../RecruitmentBoard/CampusBoardPage.css"; // 유지
import "../RecruitmentBoard/CampusBoard.css"; // 유지
...
function CampusBoard() {
  const [CampusBoard, setCampusBoard] = useState([
  ...
  const currentItems = CampusBoard.slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(CampusBoard.length / itemsPerPage);
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={{ flex: 1, padding: "70px" }}>
        {/* 검색 */}
        <table className="campus-search">
          <tbody>
            <tr>
              <td>
                <span className="campus-board-name">교내 채용공고</span>
              </td>
              <td>
                <select
                  className="campus-custom-select"
                  value={choiceVal}
              ...
              <td>
                <button type="button" className="campus-btn campus-btn-outline-secondary">
        ...
        {/* 게시판 */}
        <table className="campus-table campus-table-hover">
          <thead>
            <tr>
              <th>즐겨찾기</th>
              <th>번호</th>
        ...
        <div className="campus-pagination">
          ...
          {Array.from({ length: totalPages }, (_, index) => (
            <span key={index} className={`campus-page-number ${currentPage === index + 1 ? "active" : ""}`} onClick={() => setCurrentPage(index + 1)}>
              {index + 1}
            </span>
          ))}
        ...
        {/* 글쓰기 버튼 */}
        <div className="campus-write-container">
          <Link className="campus-card-button" to="/CampusWrite">
