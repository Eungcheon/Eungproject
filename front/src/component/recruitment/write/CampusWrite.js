...
function CampusWrite() {
  ...
  const createBbs = async () => {
    ...
    await axios
      ...
      .then((resp) => {
        console.log("[CampusWrite.js] createBbs() success :D");
        console.log(resp.data);
      ...
      .catch((err) => {
        console.log("[CampusWrite.js] createBbs() error :<");
        console.log(err);
      });
  };
  ...
  return (
    <div>
      <div className="campusWrite-d-flex">
        <Sidebar /> {/* Sidebar 추가 */}
        <div className="campusWrite-content-area"> {/* 콘텐츠 영역을 위한 div */}
          <table className="campusWrite-table">
            <tbody>
              <tr>
                <th className="campusWrite-table-primary">작성자</th>
                <td>
                  <input type="text" className="campusWrite-form-control" value={localStorage.getItem("id")} size="50px" readOnly />
                </td>
              </tr>

              <tr>
                <th className="campusWrite-table-primary">제목</th>
                <td>
                  <input type="text" className="campusWrite-form-control" value={title} onChange={changeTitle} size="50px" />
                </td>
              </tr>

              <tr>
                <th className="campusWrite-table-primary">내용</th>
                <td>
                  <textarea className="campusWrite-form-control" value={content} onChange={changeContent} rows="10"></textarea>
                </td>
              </tr>
              <tr>
                <th className="campusWrite-table-primary">파일</th>
                <td>
                  {files.map((file, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button className="campusWrite-delete-button" type="button" onClick={() => handleRemoveFile(index)}>
                        x
                      </button>
                    </div>
                  ))}
              ...
              <tr>
                <th className="campusWrite-table-primary">첨부 파일</th>
                <td>
                  {attachedFiles.map((file, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button className="campusWrite-delete-button" type="button" onClick={() => handleRemoveAttachedFile(index)}>
                        x
                      </button>
                    </div>
                  ))}
              ...
              {/* <tr>
                <th className="campusWrite-table-primary">업로드된 파일</th>
                <td>
                  {uploadedFiles.map((fileName, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button className="campusWrite-download-button" type="button" onClick={() => handleDownloadFile(fileName)}>
                        다운로드
                      </button>
                    </div>
                  ))}
          ...
          <div className="campusWrite-button-container">
  <button className="campusWrite-btn-outline-secondary" onClick={handleSubmit}>
