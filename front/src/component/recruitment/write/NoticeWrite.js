...
function NoticeWrite() {
  ...
  const createBbs = async () => {
    ...
    await axios
      ...
      .then((resp) => {
        console.log("[NoticeWrite.js] createBbs() success :D");
        console.log(resp.data);
  ...
  return (
    <div>
      <div className="noticeWrite-d-flex">
        <Sidebar /> {/* Sidebar 추가 */}
        <div className="noticeWrite-content-area"> {/* 콘텐츠 영역을 위한 div */}
          <table className="noticeWrite-table">
            <tbody>
              <tr>
                <th className="noticeWrite-table-primary">작성자</th>
                <td>
                  <input type="text" className="noticeWrite-form-control" value={localStorage.getItem("id")} size="50px" readOnly />
                </td>
              </tr>

              <tr>
                <th className="noticeWrite-table-primary">제목</th>
                <td>
                  <input type="text" className="noticeWrite-form-control" value={title} onChange={changeTitle} size="50px" />
                </td>
              </tr>

              <tr>
                <th className="noticeWrite-table-primary">내용</th>
                <td>
                  <textarea className="noticeWrite-form-control" value={content} onChange={changeContent} rows="10"></textarea>
                </td>
              </tr>
              <tr>
                <th className="noticeWrite-table-primary">파일</th>
                <td>
                  {files.map((file, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button className="noticeWrite-delete-button" type="button" onClick={() => handleRemoveFile(index)}>
                        x
                      </button>
                    </div>
                  ))}
              ...
              <tr>
                <th className="noticeWrite-table-primary">첨부 파일</th>
                <td>
                  {attachedFiles.map((file, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button className="noticeWrite-delete-button" type="button" onClick={() => handleRemoveAttachedFile(index)}>
                        x
                      </button>
                    </div>
                  ))}
              ...
              {/* <tr>
                <th className="noticeWrite-table-primary">업로드된 파일</th>
                <td>
                  {uploadedFiles.map((fileName, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button className="noticeWrite-download-button" type="button" onClick={() => handleDownloadFile(fileName)}>
                        다운로드
                      </button>
                    </div>
                  ))}
          ...
          <div className="noticeWrite-button-container">
  <button className="noticeWrite-btn-outline-secondary" onClick={handleSubmit}>
